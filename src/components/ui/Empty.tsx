import type { LucideIcon } from 'lucide-react';
import type { ReactNode } from 'react';

interface Props {
  icon: LucideIcon;
  /** Shown instead of the icon — used for the app's own mark. */
  image?: string;
  title: string;
  body?: string;
  action?: ReactNode;
}

export function Empty({ icon: Icon, image, title, body, action }: Props) {
  return (
    <div className="flex flex-col items-center justify-center px-6 py-12 text-center">
      {image ? (
        <img src={image} alt="" width={48} height={48} className="mb-3 size-12 rounded-xl opacity-90" />
      ) : (
        <Icon size={28} className="mb-3 text-ink-faint/60" aria-hidden strokeWidth={1.5} />
      )}
      <p className="text-[14px] font-medium text-ink-soft">{title}</p>
      {body && <p className="mt-1 max-w-xs text-[13px] leading-snug text-ink-faint">{body}</p>}
      {action && <div className="mt-4">{action}</div>}
    </div>
  );
}
